...
Set `API_KEY` to your organization auth key before running this example.

```bash
curl --location 'https://cerberus.eazybe.com/prod/api/v2/broadcast/public/send-template-broadcast' \
  --header "x-api-key: $API_KEY" \
  --header 'Content-Type: application/json' \
  --data '{
    "broadcastName": "broadcast 90",
    "fromPhoneNumber": "13022051228",
    "templateName": "partner_rotw",
    "templateLanguage": "en",
    "templateType": "MARKETING",
    "templateId": "972071395921578",
    "data": [
      {
        "countryCode": "91",
        "toPhoneNumber": "8882331250",
        "templateParams": [" على السداد", "الفاتورة"]
      }
    ]
  }'
```

The recipient uses the two values in `templateParams` for the template's numbered placeholders, in the same order.
